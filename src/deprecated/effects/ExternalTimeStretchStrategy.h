#ifndef EXTERNALTIMESTRETCHSTRATEGY_H
#define EXTERNALTIMESTRETCHSTRATEGY_H
...
/**
 * ExternalTimeStretchStrategy
 *
 * 외부 라이브러리(SoundTouch)를 사용한 고품질 time stretching
 *
 * 장점:
 * - 검증된 프로덕션 라이브러리 (업계 표준)
 * - 뛰어난 음질
 * - 최적화된 성능 (SIMD 지원)
 * - Phase coherence 자동 처리
 *
 * 단점:
 * - 외부 의존성 추가
 ...
 * - 외부 의존성을 허용할 수 있는 경우
 */
class ExternalTimeStretchStrategy : public ITimeStretchStrategy {
public:
    /**
     * @param antiAliasing Anti-alias filter 활성화 (기본 true)
     * @param quickSeek Quick seeking 활성화 (속도 향상, 약간의 품질 저하, 기본 false)
     */
    ExternalTimeStretchStrategy(bool antiAliasing = true, bool quickSeek = false);
    ~ExternalTimeStretchStrategy() override;

    AudioBuffer stretch(const AudioBuffer& input, float ratio) override;
    const char* getName() const override { return "ExternalTimeStretch (SoundTouch)"; }
...
#endif // EXTERNALTIMESTRETCHSTRATEGY_H
