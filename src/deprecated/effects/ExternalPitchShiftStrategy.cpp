...
AudioBuffer ExternalPitchShiftStrategy::shiftPitch(const AudioBuffer& input, float semitones) {
    ...
    // Pitch curve가 있는 경우, 해당 값 사용
    float effectiveSemitones = semitones;
    if (input.hasPitchCurve()) {
        const auto& pitchCurve = input.getPitchCurve();
        if (!pitchCurve.empty()) {
            // 평균 semitones 계산
            float sum = 0.0f;
            for (float s : pitchCurve) {
                sum += s;
            }
            effectiveSemitones = sum / pitchCurve.size();

            // TODO: 향후 개선 - SoundTouch는 고정 pitch만 지원
            // Variable pitch를 위해서는 frame-by-frame 처리 또는
            // PSOLA/Phase Vocoder 같은 알고리즘 사용 필요
        }
    }
    ...
    soundtouch_->setPitchSemiTones(effectiveSemitones);
