#include "RubberBandDurationProcessor.h"
#include <algorithm>

RubberBandDurationProcessor::RubberBandDurationProcessor(int windowSize, int hopSize)
    : algorithm_(new RubberBandDurationAlgorithm()),
      windowSize_(windowSize),
      hopSize_(hopSize) {
}

RubberBandDurationProcessor::~RubberBandDurationProcessor() {
}

std::vector<FrameData> RubberBandDurationProcessor::process(
...
std::vector<float> RubberBandDurationProcessor::processFrameByFrame(
...
float RubberBandDurationProcessor::getDurationRatioAtTime(
    float time,
    const std::vector<FrameData>& frames
) const {
...
std::vector<float> RubberBandDurationProcessor::framesToAudio(
    const std::vector<FrameData>& frames
) const {
...
std::vector<FrameData> RubberBandDurationProcessor::audioToFrames(
    const std::vector<float>& audio,
