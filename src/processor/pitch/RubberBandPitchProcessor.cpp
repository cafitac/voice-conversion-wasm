#include "RubberBandPitchProcessor.h"
#include <cmath>
#include <algorithm>

RubberBandPitchProcessor::RubberBandPitchProcessor(int windowSize, int hopSize)
    : algorithm_(new RubberBandAlgorithm(true, true)),
      frameDuration_(0.1f),  // 100ms default
      preserveFormant_(true) {
    // windowSize와 hopSize는 내부적으로 사용하지 않음 (RubberBand가 자체 처리)
    // frameDuration_은 frame-by-frame 처리 시 프레임 길이로 사용
}

RubberBandPitchProcessor::~RubberBandPitchProcessor() {
}

std::vector<FrameData> RubberBandPitchProcessor::process(
...
std::vector<float> RubberBandPitchProcessor::processFrameByFrame(
...
float RubberBandPitchProcessor::getPitchSemitonesAtTime(
    float time,
    const std::vector<FrameData>& frames
) const {
...
std::vector<float> RubberBandPitchProcessor::framesToAudio(
    const std::vector<FrameData>& frames
) const {
...
std::vector<FrameData> RubberBandPitchProcessor::audioToFrames(
    const std::vector<float>& audio,
