var Module = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    var UTF8ToString = (ptr, maxBytesToRead, ignoreNul) => ptr
        ? UTF8ArrayToString(HEAPU8, ptr, maxBytesToRead, ignoreNul) : "";
    var ___assert_fail = (condition, filename, line, func) => abort(
        `Assertion failed: ${UTF8ToString(condition)}, at: ` + [filename
            ? UTF8ToString(filename) : "unknown filename", line,
    ...
    var integerReadValueFromPointer = (name, width, signed) => {
      switch (width) {
        case 1:
          return signed ? pointer => HEAP8[pointer]
    ...
    var createNamedFunction = (name, func) => Object.defineProperty(func,
        "name", {value: name});
    ...
    function createJsInvoker(argTypes, isClassMethodFunc, returns, isAsync) {
      ...
      var args1 = ["humanName", "throwBindingError", "invoker", "fn",
        "runDestructors", "fromRetWire", "toClassParamWire"];
      if (isClassMethodFunc) {
    ...
    function craftInvokerFunction(humanName, argTypes, classType,
      ...
      var closureArgs = [humanName, throwBindingError, cppInvokerFunc,
        cppTargetFunc, runDestructors, retType.fromWireType.bind(retType),
      ...
      let invokerFactory = createJsInvoker(argTypes, isClassMethodFunc, returns,
          isAsync);
    ...
    var __embind_register_function = (name, argCount, rawArgTypesAddr,
        signature, rawInvoker, fn, isAsync, isNonnullReturn) => {
      var argTypes = heap32VectorToArray(argCount, rawArgTypesAddr);
      name = AsciiToString(name);
      name = getFunctionName(name);
      rawInvoker = embind__requireFunction(signature, rawInvoker, isAsync);
      exposePublicSymbol(name, function () {
        throwUnboundTypeError(`Cannot call ${name} due to unbound types`,
            argTypes)
      }, argCount - 1);
    ...
    var stringToUTF8 = (str, outPtr, maxBytesToWrite) => stringToUTF8Array(str,
    ...
    var __embind_register_std_string = (rawType, name) => {
      ...
      registerType(rawType, {
        name, fromWireType(value) {
          ...
          if (!(valueIsOfTypeString || ArrayBuffer.isView(value)
    ...
    var UTF16Decoder = globalThis.TextDecoder ? new TextDecoder("utf-16le")
        : undefined;
    ...
    var __emval_create_invoker = (argCount, argTypesPtr, kind) => {
      ...
      var argFromPtr = argTypes.map(
          type => type.readValueFromPointer.bind(type));
      argCount--;
      ...
      var functionName = `methodCaller<(${argTypes.map(
          t => t.name)}) => ${retType.name}>`;
    ...
    var readEmAsmArgs = (sigPtr, buf) => {
      ...
      while (ch = HEAPU8[sigPtr++]) {
        ...
        readEmAsmArgsArray.push(
            ch == 112 ? HEAPU32[buf >> 2] : ch == 106 ? HEAP64[buf >> 3] : ch
            == 105 ? HEAP32[buf >> 2] : HEAPF64[buf >> 3]);
        buf += wide ? 8 : 4
      }
    ...
    var _emscripten_asm_const_int = (code, sigPtr, argbuf) => runEmAsmFunction(
        code, sigPtr, argbuf);
    var getHeapMax = () => 2147483648;
    var alignMemory = (size, alignment) => Math.ceil(size / alignment)
        * alignment;
    var growMemory = size => {
    ...
    var bigintToI53Checked = num => num < INT53_MIN || num > INT53_MAX ? NaN
        : Number(num);
    ...
    var cwrap = (ident, returnType, argTypes, opts) => {
      var numericArgs = !argTypes || argTypes.every(
    ...
    var ASM_CONSTS = {
      14540: $0 => {
        ...
        console.log(
            "Trim update: handle=" + $0 + ", start=" + $1 + ", end=" + $2)
      }
    };
    var _malloc, _free, ___getTypeName, __emscripten_stack_restore,
        __emscripten_stack_alloc, _emscripten_stack_get_current, memory,
